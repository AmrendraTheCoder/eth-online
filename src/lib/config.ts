...
export function validateConfig(): { valid: boolean; errors: string[] } {
  ...
  if (
    config.walletConnect.projectId === "demo" ||
    !config.walletConnect.projectId
  ) {
    errors.push(
      "WalletConnect Project ID not configured. Please set NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID"
    );
  }
  ...
  if (!validLitNetworks.includes(config.lit.network)) {
    errors.push(
      `Invalid LIT network: ${
        config.lit.network
      }. Valid options: ${validLitNetworks.join(", ")}`
    );
  }
  ...
  if (config.development.nodeEnv === "production") {
    if (!config.rpc.infuraId && !config.rpc.alchemyId) {
      errors.push(
        "RPC provider required for production. Set NEXT_PUBLIC_INFURA_ID or NEXT_PUBLIC_ALCHEMY_ID"
      );
    }
  }
...
export function getDevelopmentWarnings(): string[] {
  ...
  if (config.development.nodeEnv === "development") {
    if (config.walletConnect.projectId === "demo") {
      warnings.push(
        "Using demo WalletConnect Project ID. Configure NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID for production."
      );
    }

    if (config.lit.network === "datil-dev") {
      warnings.push(
        "Using LIT development network. This is not recommended for production."
      );
    }

    if (!config.rpc.infuraId && !config.rpc.alchemyId) {
      warnings.push(
        "No RPC provider configured. Some features may not work properly."
      );
    }
  }
