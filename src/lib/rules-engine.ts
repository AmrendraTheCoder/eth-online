...
class RulesEngine {
   ...
   * Check if rule conditions are met
   */
  private checkRuleConditions(rule: AirdropRule, context: RuleExecutionContext): boolean {
    try {
      ...
      if (rule.trigger === 'new_airdrop' && context.airdropOpportunity) {
        ...
        if (rule.condition.includes(`project = '${opp.project.toLowerCase()}'`)) {
          return true;
        }
   ...
   */
  private async createExecutionContext(rule: AirdropRule): Promise<RuleExecutionContext> {
   ...
   * Check if rule should execute
   */
  private shouldExecuteRule(rule: AirdropRule, context: RuleExecutionContext): boolean {
  ...
  getRuleStats(): {
    ...
    const activeRules = this.getActiveRules();
    const totalExecutions = allRules.reduce((sum, rule) => sum + rule.executions, 0);
    const averageExecutionsPerRule = allRules.length > 0 ? totalExecutions / allRules.length : 0;
    
    const mostExecutedRule = allRules.reduce((max, rule) => 
      rule.executions > max.executions ? rule : max, 
...
export const addRule = (rule: Omit<AirdropRule, 'id' | 'executions'>) => rulesEngine.addRule(rule);
...
export const updateRule = (ruleId: string, updates: Partial<AirdropRule>) => rulesEngine.updateRule(ruleId, updates);
export const deleteRule = (ruleId: string) => rulesEngine.deleteRule(ruleId);
export const executeRule = (ruleId: string, context: RuleExecutionContext, pkpTokenId?: string) => 
  rulesEngine.executeRule(ruleId, context, pkpTokenId);
export const startRuleMonitoring = (intervalMs?: number) => rulesEngine.startMonitoring(intervalMs);
export const stopRuleMonitoring = () => rulesEngine.stopMonitoring();
export const getRuleExecutionHistory = (ruleId: string) => rulesEngine.getRuleExecutionHistory(ruleId);
